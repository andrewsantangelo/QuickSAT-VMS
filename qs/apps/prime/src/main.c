...
void db_update(db_conn_info_t *conn, char *param, uint32_t val) {
    ...
    if (NULL != conn->param_update_stmt) {
        ...
        if (0 != mysql_stmt_bind_param(conn->param_update_stmt, bind)) {
            printf("ERROR: unable to bind (%s, %u) values to update parameter MySQL statement (%s)\n",
                    param, val, mysql_error(conn->mysql));
        } else {
            if (0 != mysql_stmt_execute(conn->param_update_stmt)) {
                printf("ERROR: unable to execute update parameter MySQL statement for params (%s, %u) (%s)\n",
                        param, val, mysql_error(conn->mysql));
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        while (!found) {
            ...
            for (root = val - 1; root > 1; root--) {
                if (0 == (val % root)) {
                    break;
                }
            }

            /* If no root divides evenly, a prime number was found.
             * (use >= to guard against the case where the value
             * being checked is 1 so the first root is 0) */
            if (1 >= root) {
                found = true;
            }
        }
